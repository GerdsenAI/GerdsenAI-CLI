    ...
    def is_command_allowed(self, command: str) -> tuple[bool, str]:
        ...
        if cmd_base in self.blocked_commands:
            if self.safety_level == "strict":
                ...
                return (
                    True,
                    f"Command '{cmd_base}' requires confirmation (security risk)",
                )
            ...
            for blocked_dir in self.blocked_directories:
                try:
                    ...
                    if work_path.is_relative_to(blocked_path):
                        return (
                            False,
                            f"Directory '{working_dir}' is in blocked path '{blocked_dir}'",
                        )
...
class TerminalExecutor:
    ...
    FILE_OPERATION_COMMANDS = {"cp", "mv", "copy", "move", "rename", "ln", "link"}
    ...
    async def execute_command(
        ...
        dir_allowed, dir_message = self.security_config.check_directory_boundaries(
            work_dir
        )
        if not dir_allowed:
        ...
        if require_confirmation:
            ...
            needs_confirmation = (
                risk_level != "MINIMAL"
                or "requires confirmation" in security_message.lower()
            )

            if needs_confirmation and not await self._get_user_confirmation(
                command, risk_level
            ):
    ...
    def format_command_output(self, result: CommandResult) -> None:
        ...
        if result.stderr.strip() and not result.success:
            console.print(
                Panel(result.stderr.strip(), title="Error Output", border_style="red")
            )
