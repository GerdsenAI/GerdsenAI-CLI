...
class CapabilityDetector:
    ...
    def detect_from_model_name(model_name: str) -> ModelCapabilities:
        ...
        supports_thinking = any(
            known in model_lower 
            for known in CapabilityDetector.THINKING_MODELS
        )
        
        # Check for vision support
        supports_vision = any(
            known in model_lower 
            for known in CapabilityDetector.VISION_MODELS
        )
        
        # Check for tool calling
        supports_tools = any(
            known in model_lower 
            for known in CapabilityDetector.TOOL_MODELS
        )
    ...
    async def probe_capabilities(agent) -> ModelCapabilities:
        ...
        try:
            ...
            model_name = getattr(agent, 'model_name', None) or \
                        getattr(agent, 'model', None)
            
            if not model_name and hasattr(agent, 'settings'):
                model_name = getattr(agent.settings, 'current_model', None) or \
                            getattr(agent.settings, 'model', None)
