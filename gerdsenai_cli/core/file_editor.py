...
class DiffGenerator:
    ...
    def format_diff_for_display(
        diff_text: str, syntax_theme: str = "monokai", show_line_numbers: bool = True
    ) -> Syntax:
    ...
    def _cleanup_old_backups(self, max_age_days: int = 7, max_count: int = 100) -> None:
        ...
        try:
            ...
            for i, backup_file in enumerate(backup_files):
                file_age_days = (current_time - backup_file.stat().st_mtime) / (
                    24 * 3600
                )
...
class FileEditor:
    ...
    def __init__(self, backup_manager: BackupManager | None = None):
        ...
        self.current_session: EditSession | None = None
    ...
    async def propose_edit(
        ...
        try:
            ...
            if file_path.exists() and operation != EditOperation.CREATE:
                try:
                    ...
                    for encoding in ["utf-8-sig", "latin-1", "cp1252"]:
                        ...
                        show_error(
                            f"Cannot read file {file_path} - unsupported encoding"
                        )
                        return None
            ...
            # Type guard to ensure current_session is not None after start_session()
            if self.current_session:
    ...
    async def apply_edit(self, edit: FileEdit, force: bool = False) -> bool:
        ...
        try:
            ...
            if self.require_confirmation and not force:
                ...
                # Enhanced confirmation for destructive operations
                if self._is_destructive_operation(edit):
                    if not await self._confirm_destructive_operation(edit):
    ...
    async def _execute_edit(self, edit: FileEdit) -> bool:
        ...
        try:
            if (
                edit.operation == EditOperation.CREATE
                or edit.operation == EditOperation.MODIFY
            ):
                # Ensure parent directory exists
                edit.target_path.parent.mkdir(parents=True, exist_ok=True)

                # Write new content (guard against None)
            ...
            if edit.operation == EditOperation.CREATE:
                ...
                if edit.backup_path and edit.backup_path.exists():
                    success = self.backup_manager.restore_backup(
                        edit.backup_path, edit.target_path
                    )
                    if success:
    ...
    async def apply_session_edits(
        ...
        show_info(
            f"Edit session complete: {applied_count} applied, {failed_count} failed"
        )
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        ...
        return [
            self.get_session_summary(session_id) for session_id in self.sessions.keys()
        ]
