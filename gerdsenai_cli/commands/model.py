...
class ListModelsCommand(BaseCommand):
    ...
    async def execute(self, args: Dict[str, Any], context: Any = None) -> CommandResult:
        ...
        try:
            ...
            if filter_pattern:
                models = [m for m in models if filter_pattern.lower() in m.get("name", "").lower()]
                if not models:
                    console.print(f"[yellow]No models found matching pattern: {filter_pattern}[/yellow]")
                    return CommandResult(success=True, message=f"No models matching '{filter_pattern}'")
    ...
    async def _display_simple_models(self, console: Console, models: List[Dict], context: Any):
    ...
    async def _display_detailed_models(self, console: Console, models: List[Dict], context: Any):
        ...
        for i, model in enumerate(models):
            ...
            details.append(f"[bold]Status:[/bold] {'🟢 Currently Active' if is_current else '⚪ Available'}")
            ...
            if "details" in model:
                ...
                if "parameter_size" in model_details:
                    details.append(f"[bold]Parameters:[/bold] {model_details['parameter_size']}")
                if "quantization_level" in model_details:
                    details.append(f"[bold]Quantization:[/bold] {model_details['quantization_level']}")
    ...
    async def execute(self, args: Dict[str, Any], context: Any = None) -> CommandResult:
        ...
        try:
            ...
            if model_name == current_model:
                ...
                return CommandResult(success=True, message=f"Already using {model_name}")
            ...
            if not force:
                ...
                if model_name not in model_names:
                    ...
                    return CommandResult(success=False, message=f"Model '{model_name}' not found")
            ...
            console.print(f"[green]✓[/green] Switched from [cyan]{old_model or 'none'}[/cyan] to [cyan]{model_name}[/cyan]")
...
class ModelInfoCommand(BaseCommand):
    ...
    async def execute(self, args: Dict[str, Any], context: Any = None) -> CommandResult:
        ...
        try:
            config = ConfigManager()
            model_name = args.get("model_name") or config.get_setting("current_model", "")
    ...
    async def _display_model_info(self, console: Console, model: Dict, config: ConfigManager):
        ...
        info_lines.append(f"  Status: {'Currently Active' if is_current else 'Available'}")
    ...
    async def execute(self, args: Dict[str, Any], context: Any = None) -> CommandResult:
        ...
        try:
            ...
            if not stats:
                ...
                console.print("[dim]Statistics will be collected as you use models.[/dim]")
                return CommandResult(success=True, message="No statistics available")
            
            # Display statistics
            table = Table(title="Model Usage Statistics", show_header=True, header_style="bold magenta")
