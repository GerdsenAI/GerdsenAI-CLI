...
class CommandArgument:
    ...
    def validate(self, value: str) -> Any:
        ...
        if self.choices and value not in self.choices:
            raise ValueError(
                f"Invalid choice '{value}'. Must be one of: {', '.join(self.choices)}"
            )

        try:
            if self.arg_type is bool:
                return value.lower() in {"true", "1", "yes", "on"}
            elif self.arg_type is int:
                return int(value)
            elif self.arg_type is float:
                return float(value)
            else:
                return str(value)
        except (ValueError, TypeError) as e:
            raise ValueError(
                f"Cannot convert '{value}' to {self.arg_type.__name__}: {e}"
            ) from e
...
class BaseCommand(ABC):
    ...
    def __init__(self):
        ...
        self._args_cache: dict[str, CommandArgument] | None = None
    ...
    def usage(self) -> str:
        ...
        for arg_name, arg_def in self.arguments.items():
            if arg_def.required:
                ...
                default_str = (
                    f"={arg_def.default}" if arg_def.default is not None else ""
                )
    ...
    def help_text(self) -> str:
        ...
        if self.arguments:
            ...
            for arg_name, arg_def in self.arguments.items():
                ...
                default_str = (
                    f" [default: {arg_def.default}]"
                    if arg_def.default is not None
                    else ""
                )
                type_str = f" [{arg_def.arg_type.__name__}]"
                choices_str = (
                    f" (choices: {', '.join(arg_def.choices)})"
                    if arg_def.choices
                    else ""
                )

                lines.append(
                    f"  `{arg_name}`{type_str}{req_str} - {arg_def.description}{default_str}{choices_str}"
                )
        ...
        for i, arg_def in enumerate(positional_args):
            if i < len(raw_args):
                try:
                    ...
                    raise ValueError(f"Error in argument '{arg_def.name}': {e}") from e
            else:
            ...
            if "=" in arg_text:
                ...
                if key in self.arguments:
                    try:
                        ...
                        raise ValueError(f"Error in argument '{key}': {e}") from e
                else:
                    raise ValueError(f"Unknown argument: {key}")
            else:
                # Treat as boolean flag if it's defined as boolean argument
                if (
                    arg_text in self.arguments
                    and self.arguments[arg_text].arg_type is bool
                ):
    ...
    async def run(self, args_text: str, context: dict[str, Any]) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                success=False, message=f"Argument error: {e}\n\nUsage: {self.usage}"
            )
    ...
    async def execute(
        ...
        try:
            ...
            if isinstance(result, CommandResult):
                ...
                return CommandResult(
                    success=True, message=str(result) if result else None
                )
        except Exception as e:
