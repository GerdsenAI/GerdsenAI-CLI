...
class CommandRegistry:
    ...
    def register(self, command: BaseCommand) -> None:
        ...
        for alias in command.aliases:
            if alias in self.aliases or alias in self.commands:
                raise ValueError(f"Alias '{alias}' conflicts with existing command or alias")
    ...
    def unregister(self, command_name: str) -> bool:
        ...
        # Remove aliases
        aliases_to_remove = [alias for alias, name in self.aliases.items() if name == command_name]
        ...
        if command.category in self.categories:
            self.categories[command.category] = [
                name for name in self.categories[command.category] if name != command_name
            ]
...
class CommandParser:
    ...
    def parse_command(self, input_text: str) -> Optional[tuple[str, str]]:
        """
    ...
    def _get_command_suggestions(self, command_name: str, max_suggestions: int = 3) -> List[str]:
        """
        ...
        suggestions.sort(key=lambda x: self._string_similarity(command_name.lower(), x.lower()), reverse=True)
    ...
    def _show_general_help(self) -> None:
        ...
        for category in CommandCategory:
            ...
            for command in sorted(commands, key=lambda c: c.name):
                ...
                if command.aliases:
                    cmd_text += f" ({', '.join(f'/{alias}' for alias in command.aliases)})"
        ...
        console.print("🚀 [dim]Your AI can understand your project and perform intelligent actions![/dim]\n")
    ...
    def show_status(self) -> None:
        ...
        console.print(f"  Context Keys:        [bold]{len(status['context_keys'])}[/bold]")
    ...
    def discover_from_classes(self, classes: List[Type[BaseCommand]]) -> int:
        ...
        for command_class in classes:
            try:
                ...
                show_warning(f"Failed to register command class {command_class.__name__}: {e}")
