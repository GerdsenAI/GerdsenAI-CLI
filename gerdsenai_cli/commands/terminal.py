...
class HistoryCommand(BaseCommand):
    ...
    def __init__(self, **kwargs):
        ...
        self.terminal_executor = getattr(
            kwargs.get("run_command"), "terminal_executor", None
        )
        ...
        try:
            ...
            if args.strip():
                try:
                    ...
                    if limit <= 0:
                        show_warning(
                            "Limit must be a positive number, showing all history"
                        )
                        limit = None
                except ValueError:
            ...
            for i, result in enumerate(history, 1):
                ...
                table.add_row(
                    str(i), command_display, status, exit_code, exec_time, timestamp
                )
...
class TerminalStatusCommand(BaseCommand):
    ...
    async def execute(self, args: str) -> dict[str, Any]:
        ...
        try:
            ...
            status_text.append(
                "Windows" if self.terminal_executor.is_windows else "Unix-like",
                style="blue",
            )

            panel = Panel(
                status_text, title="Terminal Executor Status", border_style="blue"
            )
            ...
            return {
                ...
                "platform": "Windows"
                if self.terminal_executor.is_windows
                else "Unix-like",
            }
