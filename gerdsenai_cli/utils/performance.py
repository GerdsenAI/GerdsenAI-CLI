...
class PerformanceTracker:
    ...
    def get_startup_duration(self) -> Optional[float]:
    ...
    def get_metrics(
        ...
        if since:
            ...
            filtered_metrics = [m for m in filtered_metrics if m.start_time >= since_timestamp]
    ...
    def get_operation_stats(self, operation: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "target_met": all(d <= self.targets.get(operation, float('inf')) for d in durations)
        }
    ...
    def check_performance_targets(self) -> Dict[str, bool]:
        ...
        if self.startup_time:
            ...
            if startup_duration:
                results["startup_time"] = startup_duration <= self.targets["startup_time"]
    ...
    def display_performance_report(self, show_details: bool = False) -> None:
        ...
        if not self.metrics:
            console.print(Panel("No performance metrics available", title="Performance Report"))
            return
        ...
        memory_status = "✓" if current_memory <= self.targets["memory_baseline"] else "✗"
        memory_color = "green" if current_memory <= self.targets["memory_baseline"] else "red"
        ...
        if self.startup_time:
            startup_duration = self.get_startup_duration()
            if startup_duration:
                startup_status = "✓" if startup_duration <= self.targets["startup_time"] else "✗"
                startup_color = "green" if startup_duration <= self.targets["startup_time"] else "red"
                console.print(f"Startup Time: [{startup_color}]{startup_duration:.3f}s[/{startup_color}] "
...
def log_slow_operation(operation: str, duration: float, threshold: float = 1.0) -> None:
    ...
    if duration > threshold:
        ...
        if target and duration > target:
            console.print(f"[yellow]Warning: {operation} took {duration:.3f}s "
