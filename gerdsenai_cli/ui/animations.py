...
class PlanCapture:
    ...
    def extract_summary(full_response: str) -> dict:
        ...
        for line in lines:
            ...
            if any(ext in line for ext in ['.py', '.js', '.ts', '.json', '.md', '.txt', '.yml', '.yaml', '.toml']):
                # Extract filename
                for word in line.split():
                    if any(ext in word for ext in ['.py', '.js', '.ts', '.json', '.md', '.txt', '.yml', '.yaml', '.toml']):
                        clean_word = word.strip('`,"\'()[]{}:;')
                        if clean_word and len(clean_word) > 3:
                            files_affected.append(clean_word)
            
            # Extract actions (lines mentioning key action verbs)
            if any(word in line_lower for word in ['create', 'modify', 'delete', 'update', 'add', 'remove', 'implement', 'refactor']):
            ...
            if line.strip() and len(summary_lines) < 5:
                ...
                if not clean_line.startswith('#') and len(clean_line) > 20:
                    ...
                    if not any(marker in clean_line for marker in ['```', '---', '===', '***']):
        ...
        return {
            "summary": '\n'.join(summary_lines[:3]) if summary_lines else "AI planning response",
            "files_affected": list(dict.fromkeys(files_affected))[:10],  # Dedupe and limit
